...
data class ParseResult(val cleanText: String, val timestamp: Long?, val hasTime: Boolean)
...
object SmartDateParser {
    fun parse(input: String): ParseResult {
        ...
        // 3. Time: "at 5pm", "5:30 am", "5 am", "at 14:00", "at 5"
        if (!hasTime) {
        ...
        if (!hasDate) {
            ...
            val monthRegex = Regex("(?i)\\b(?:on\\s+)?(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\\s+(\\d{1,2})(?:st|nd|rd|th)?\\b")
            monthRegex.find(text)?.let { match ->
                ...
                val dayMonthRegex = Regex("(?i)\\b(?:on\\s+)?(\\d{1,2})(?:st|nd|rd|th)?\\s+(?:of\\s+)?(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\\b")
        ...
        if (hasDate && !hasTime) {
            ...
            return ParseResult(input.trim(), null, false)
        }
        
        return ParseResult(text.trim(), cal.timeInMillis, hasTime)
    }
}
